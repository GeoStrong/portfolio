...
const Centerpiece = () => {
  ...
  useFrame((state) => {
    ...
    meshRef.current.rotation.x = THREE.MathUtils.lerp(meshRef.current.rotation.x, baseRotationX + mouse.y * 0.08, 0.05);
    meshRef.current.rotation.y = THREE.MathUtils.lerp(meshRef.current.rotation.y, baseRotationY + mouse.x * 0.08, 0.05);
    
    const pulse = 1 + Math.sin(time * 0.5) * 0.03;
    const baseScale = hovered ? 1.25 : 1.1;
    const finalScale = baseScale * pulse;
    meshRef.current.scale.setScalar(THREE.MathUtils.lerp(meshRef.current.scale.x, finalScale, 0.06));
  });
